...
next_stage_text = font1.render("NEXT STAGE", True, black)
...
next_stage_rect = next_stage_text.get_rect(center=(WIDTH // 2, 300))
...
def show_game_menu(window):
    global stage_1_active, main_menu, click_sound_volume, BGM_sound_volume, stage_2_active, previous_screen
    game_menu = True
    setting_screen = False
    ...
    while run:
            ...
            if event.type == pygame.QUIT:
                ...
                if game_menu:
                    if continue_rect.collidepoint(mouse_x, mouse_y):
                        ...
                        click_volume.play()
                        print("게임이 이어집니다")
                    elif next_stage_rect.collidepoint(mouse_x, mouse_y):
                        click_volume.play()
                        stage_1_active = False
                        stage_2_active = True
        ...
        if game_menu:
            ...
            window.blit(next_stage_text, next_stage_rect)
            ...
            window.blit(BGM_volume_value_text,
                        (WIDTH // 2 - 20 - offset_x + offset2_x, 240))  # y 좌표를 변경해 볼륨 표시 위치를 조정합니다.
...
class Block_Obstacle_10(Object):
    def __init__(self, x, y, size):
        super().__init__(x, y, size, size)
        block_obstacle_10 = get_block_obstacle_10(size)
        self.image.blit(block_obstacle_10, (0, 0))
        self.mask = pygame.mask.from_surface(self.image)
...
def stage_1(window):
    ...
    objects = [Block_Obstacle_3(block_size * 3, HEIGHT - block_size * 3, block_size),
               ...
               Block_Obstacle_10(block_size * 28, HEIGHT - block_size * 2, block_size)]
